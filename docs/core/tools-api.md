...
- **Tool (`tools.ts`):** An interface and base class (`BaseTool`) that defines the contract for all tools. Each tool must have:
  - `name`: A unique internal name (used in API calls to Salesforce LLM Gateway).
  - `displayName`: A user-friendly name.
  - `description`: A clear explanation of what the tool does, which is provided to the Salesforce LLM Gateway.
  - `parameterSchema`: A JSON schema defining the parameters that the tool accepts. This is crucial for the Salesforce LLM Gateway to understand how to call the tool correctly.
...
- **Tool Registry (`tool-registry.ts`):** A class (`ToolRegistry`) responsible for:
  ...
  - **Providing Schemas:** Exposing the `FunctionDeclaration` schemas of all registered tools to the Salesforce LLM Gateway, so it knows what tools are available and how to use them.
...
1.  **Model Request:** The Salesforce LLM Gateway, based on the user's prompt and the provided tool schemas, decides to use a tool and returns a `FunctionCall` part in its response, specifying the tool name and arguments.
...
8.  **Response to Model:** The `llmContent` from the `ToolResult` is packaged as a `FunctionResponse` and sent back to the Salesforce LLM Gateway so it can continue generating a user-facing response.
