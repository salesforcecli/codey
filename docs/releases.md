...
The workflow will then run `npm dist-tag add` for both the `@salesforce/codey` and `@salesforce/codey-core` packages, pointing the specified channel to the specified version.
...
- `npx -y @salesforce/codey@latest --version` to validate the push worked as expected if you were not doing a rc or dev tag
- `npx -y @salesforce/codey@<release tag> --version` to validate the tag pushed appropriately
- _This is destructive locally_ `npm uninstall @salesforce/codey && npm uninstall -g @salesforce/codey && npm cache clean --force &&  npm install @salesforce/codey@<version>`
