...
- `prompt` (String): The prompt that will be sent to the Salesforce LLM Gateway when the command is executed. This can be a single-line or multi-line string.
...
- **`@<path_to_file_or_directory>`**
  ...
  - **Details:**
    ...
    - The command uses the `read_many_files` tool internally. The content is fetched and then inserted into your query before being sent to the Salesforce LLM Gateway.
...
- **`@` (Lone at symbol)**
  - **Description:** If you type a lone `@` symbol without a path, the query is passed as-is to the Salesforce LLM Gateway. This might be useful if you are specifically talking _about_ the `@` symbol in your prompt.

### Error handling for `@` commands

- If the path specified after `@` is not found or is invalid, an error message will be displayed, and the query might not be sent to the Salesforce LLM Gateway, or it will be sent without the file content.
