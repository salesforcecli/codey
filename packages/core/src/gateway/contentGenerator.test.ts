...
const setMockClient = (gen: GatewayContentGenerator, client: MockClient) => {
...
describe('GatewayContentGenerator (happy paths)', () => {
  ...
  describe('generateContent', () => {
    it('sends a properly constructed request and returns candidates with usage', async () => {
      ...
      expect(resp.candidates?.[0]?.content?.parts?.[0]).toEqual({
        text: 'Pong',
      });
  ...
  describe('generateContentStream', () => {
    it('yields text and then a terminal chunk with usage', async () => {
      ...
      expect(first.candidates?.[0]?.content?.parts?.[0]).toEqual({
        text: 'Hello',
      });
