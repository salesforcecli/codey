...
export interface ReadManyFilesParams {
  ...
  /**
   * Whether to respect .gitignore and .codeyignore patterns (optional, defaults to true)
   */
 ...
 * This combines the shared patterns with dynamic patterns like CODEY.md.
...
${finalExclusionPatternsForDescription
  ...
  .join(
    ...
    // Add a note if .codeyignore patterns contributed to the final list of exclusions
    if (geminiIgnorePatterns.length > 0) {
      const geminiPatternsInEffect = geminiIgnorePatterns.filter((p) =>
        finalExclusionPatternsForDescription.includes(p),
      ).length;
      if (geminiPatternsInEffect > 0) {
        excludeDesc += ` (includes ${geminiPatternsInEffect} from .codeyignore)`;
      }
    }
  ...
  constructor(private config: Config) {
    const parameterSchema = {
      ...
      properties: {
        ...
        file_filtering_options: {
          description:
            'Whether to respect ignore patterns from .gitignore or .codeyignore',
          type: 'object',
          properties: {
            respect_git_ignore: {
              description:
            ...
            respect_gemini_ignore: {
              description:
                'Optional: Whether to respect .codeyignore patterns when listing files. Defaults to true.',
              type: 'boolean',
            },
          },
        },
